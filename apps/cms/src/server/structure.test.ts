...
beforeAll(async () => {
	...
	const rows = [
		section(S1, DOC, { ...numbered, address: '1', printedNumber: '1', title: 'Prevention', headingLevel: 1, orderIndex: 1 }),
		section(S11, DOC, { ...numbered, parentId: S1, address: '1.1', printedNumber: '1.1', title: 'Risk', headingLevel: 2, orderIndex: 1 }),
		section(S111, DOC, { ...numbered, parentId: S11, address: '1.1.1', printedNumber: '1.1.1', title: 'Smoking', headingLevel: 3, orderIndex: 1 }),
		section(S12, DOC, { ...numbered, parentId: S1, address: '1.2', printedNumber: '1.2', title: 'Screening', headingLevel: 2, orderIndex: 2 }),
		section(S2, DOC, { canonical: true, stepNumber: 2, address: '2', printedNumber: '2', title: 'Presentation', headingLevel: 1, orderIndex: 2 }),
		section(S2T, DOC, { stepNumber: 2, parentId: S2, address: '2/about', title: 'About this step', headingLevel: 2, orderIndex: 1 }),
		...
		// publish, Q2 after.
		section(P1, REPORT_DOC, { canonical: true, address: '1', printedNumber: '1', title: 'One', orderIndex: 1, hidden: true }),
		section(P2, REPORT_DOC, { canonical: true, address: '2', printedNumber: '2', title: 'Two', orderIndex: 2, hidden: true }),
		section(P3, REPORT_DOC, { canonical: true, address: '3', printedNumber: '3', title: 'Three', orderIndex: 3 }),
		section(P4, REPORT_DOC, { canonical: true, address: '4', printedNumber: '4', title: 'Four', orderIndex: 4, hidden: true }),
		section(Q1, REPORT_DOC, { added: true, parentId: P3, address: '3/earlier', title: 'Earlier', orderIndex: 1 }),
		section(Q2, REPORT_DOC, { added: true, parentId: P3, address: '3/later', title: 'Later', orderIndex: 2 }),
	]
	// D1 binds at most 100 parameters to a statement: a few rows at a time.
	for (let i = 0; i < rows.length; i += 4) await d.insert(schema.sections).values(rows.slice(i, i + 4))
...
describe('hiding and showing', () => {
	it('hides a section with its whole subtree, and shows it all again', async () => {
		const hidden = await st.setSectionHidden(lifecycle(), { sectionId: S1, hidden: true, userId: MEMBER })
		...
		const shown = await st.setSectionHidden(lifecycle(), { sectionId: S1, hidden: false, userId: MEMBER })
		...
		expect(events.map((e) => e.kind)).toEqual(expect.arrayContaining(['section.hidden', 'section.shown']))
	})
...
describe('the numbered spine', () => {
	it('refuses renaming a numbered section at the door and in the database', async () => {
		...
		expect(
			await refusal(d.update(schema.sections).set({ title: 'Risk factors' }).where(eq(schema.sections.id, S11))),
		).toMatch(/numbered by the template/)
		expect(
			await refusal(d.update(schema.sections).set({ orderIndex: 5 }).where(eq(schema.sections.id, S11))),
		).toMatch(/numbered by the template/)
...
describe('added subheadings', () => {
	...
	it('adds first, between and last, under unique addresses, indexed for search', async () => {
		const a = await st.addSubsection(lifecycle(), { parentId: S1, title: 'Local services', afterId: null, userId: MEMBER })
		const b = await st.addSubsection(lifecycle(), { parentId: S1, title: ' Local services ', afterId: S11, userId: MEMBER })
		const c = await st.addSubsection(lifecycle(), { parentId: S1, title: 'Follow-up’s timing', afterId: S12, userId: MEMBER })
		first = a.id
		between = b.id
		last = c.id
		...
		const events = await db(env.DB).select().from(schema.events).where(eq(schema.events.documentId, DOC))
	...
	it('refuses an empty or overlong title, a sibling from elsewhere, scaffolding and a viewer', async () => {
		...
		await expect(
			st.addSubsection(lifecycle(), { parentId: S1, title: 'x'.repeat(201), afterId: null, userId: MEMBER }),
		).rejects.toThrow(/at most 200/)
		await expect(
			st.addSubsection(lifecycle(), { parentId: S1, title: 'Elsewhere', afterId: S2T, userId: MEMBER }),
		).rejects.toThrow(/not under this heading/)
		await expect(
			st.addSubsection(lifecycle(), { parentId: APP, title: 'Cover note', afterId: null, userId: MEMBER }),
		).rejects.toThrow(/scaffolding/)
		await expect(
			st.addSubsection(lifecycle(), { parentId: S1, title: 'Viewer note', afterId: null, userId: VIEWER }),
		).rejects.toThrow(/drafter or above/)
	})

	it('renames an added subheading, keeping its address, and reindexes it', async () => {
		await st.renameSection(lifecycle(), { sectionId: between, title: 'Services nearby', userId: MEMBER })
	...
	it('moves by midpoints, carries its subtree and the new step, refuses cycles and template sections', async () => {
		const child = await st.addSubsection(lifecycle(), { parentId: first, title: 'Clinics', afterId: null, userId: MEMBER })
		...
		const moved = await st.moveSection(lifecycle(), { sectionId: first, parentId: S1, beforeId: S12, userId: MEMBER })
		expect(moved.orderIndex).toBe(1.75)
		// Under step 2, before its template section: one before it.
		const across = await st.moveSection(lifecycle(), { sectionId: first, parentId: S2, beforeId: S2T, userId: MEMBER })
		expect(across.orderIndex).toBe(0)
		const [row, childRow] = await Promise.all([rowOf(first), rowOf(child.id)])
		expect(row).toMatchObject({ parentId: S2, stepNumber: 2, headingLevel: 2, address: '1/local-services' })
		...
		const deeper = await st.moveSection(lifecycle(), { sectionId: first, parentId: S12, beforeId: null, userId: MEMBER })
		...
		await expect(
			st.moveSection(lifecycle(), { sectionId: first, parentId: child.id, beforeId: null, userId: MEMBER }),
		).rejects.toThrow(/under itself/)
		await expect(
			st.moveSection(lifecycle(), { sectionId: first, parentId: first, beforeId: null, userId: MEMBER }),
		).rejects.toThrow(/under itself/)
		await expect(
			st.moveSection(lifecycle(), { sectionId: S12, parentId: S2, beforeId: null, userId: MEMBER }),
		).rejects.toThrow(/numbered by the template/)
		await expect(
			st.moveSection(lifecycle(), { sectionId: first, parentId: P3, beforeId: null, userId: MEMBER }),
		).rejects.toThrow(/not in this document/)
		await expect(
			st.moveSection(lifecycle(), { sectionId: first, parentId: S1, beforeId: S2T, userId: MEMBER }),
	...
	it('refuses deleting a subheading holding a template section; deletes one with its own subtree', async () => {
		...
		await expect(st.deleteSection(lifecycle(), { sectionId: last, userId: MEMBER })).rejects.toThrow(
			/Stray.*comes from the template/,
		)
		...
		expect(await d.select().from(schema.comments).where(eq(schema.comments.sectionId, childId))).toEqual([])
...
describe('the structure report', () => {
	...
	it('names them in the review request', async () => {
		...
		const result = await lc.requestReview(lifecycle(), { documentId: REPORT_DOC, userId: MEMBER, note: null })
