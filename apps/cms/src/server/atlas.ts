...
function tickStates(
	...
	for (const s of sections) {
		...
		if (!published?.has(s.id)) out.set(s.id, 'new')
			...
			out.set(
				s.id,
				s.draftHash !== null && s.draftHash === published.get(s.id) ? 'published' : 'changed',
			)
	}
	return out
}
...
export const atlasSnapshot = createServerFn({ method: 'GET' }).handler(
	async ({ context }): Promise<AtlasSnapshot> => {
			...
			central
				...
				: inGroups(ids, (group) =>
						d
							...
							.where(inArray(schema.sections.documentId, group)),
					),
			inGroups(ids, (group) =>
				d
					.select({
					...
					.where(
						and(
							inArray(schema.versions.documentId, group),
							inArray(schema.versions.status, ['draft', 'published']),
						),
					),
			),
			inGroups(ids, (group) =>
		...
		const reviews = await inGroups(
			...
			(group) =>
				d
					...
					.where(
						and(inArray(schema.reviewState.versionId, group), eq(schema.reviewState.superseded, 0)),
					),
		)
		const reviewSections = await inGroups(
			...
			(group) =>
				d
					...
					.where(inArray(schema.reviewSections.reviewId, group)),
		)
		...
		for (const r of reviews)
			decisionsOf.set(
				...
				new Map(
					reviewSections
						.filter((s) => s.reviewId === r.reviewId)
						.map((s) => [s.sectionId, s.decision]),
				),
			)
		...
		const people = await env.AUTH.getUsersByIds([
			...new Set([...lastOf.values()].flatMap((l) => (l.by ? [l.by] : []))),
		])
		...
		const rows: AtlasRow[] = documents.map(({ row, role }) => {
			...
			const hashes = published
				? new Map((hashesOf.get(row.id) ?? []).map((h) => [h.sectionId, h.bodyHash]))
				: null
			...
			return {
				...
				review: review
					? { total: review.total, decided: review.decided, decision: review.decision }
					: null,
				lastChange: last
					? { at: last.at, by: last.by ? (people[last.by]?.name ?? null) : null }
					: null,
				bands,
			}
		})
...
export const atlasCell = createServerFn({ method: 'GET' })
	.inputValidator(
		z.object({
			...
			band: z.union([z.literal('front'), z.literal('back'), z.number().int().min(1).max(7)]),
		}),
	)
	.handler(async ({ data, context }): Promise<AtlasCellSection[]> => {
		...
		if (!document || !(await access.documentRoleOf(env.AUTH, d, userId, document.orgId)))
		...
		const published = (
			await d
				...
				.where(
					and(
						eq(schema.versions.documentId, data.documentId),
						eq(schema.versions.status, 'published'),
					),
				)
				.limit(1)
		...
		const draft = (
			await d
				...
				.from(schema.versions)
				.where(
					and(eq(schema.versions.documentId, data.documentId), eq(schema.versions.status, 'draft')),
				)
				.limit(1)
		)[0]
		const review = draft
			? (
					await d
						...
						.from(schema.reviewState)
						.where(
							and(eq(schema.reviewState.versionId, draft.id), eq(schema.reviewState.superseded, 0)),
						)
		...
		const people = await env.AUTH.getUsersByIds([
			...new Set(band.sections.flatMap((s) => (s.updatedBy ? [s.updatedBy] : []))),
		])
...
export const publicLibrary = createServerFn({ method: 'GET' }).handler(
	async (): Promise<LibraryEntry[]> => {
		...
		const accents = new Map(
			(
				await d
					.select({ id: schema.documents.id, accent: schema.documents.accent })
					.from(schema.documents)
			).map((a) => [a.id, a.accent]),
		)
		return published
...
export const jumpIndex = createServerFn({ method: 'GET' }).handler(
	async ({ context }): Promise<JumpDocument[]> => {
		const { d } = await envOf()
		const published = await d.select().from(schema.publishedVersions)
