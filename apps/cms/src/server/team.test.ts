...
const person = (key: string, name: string): Person => ({ id: `${key}-${run}`, name, email: `${key}-${run}@example.org` })
...
class MemoryAuth implements team.TeamAuth, team.TeamDoors {
	...
	invitations = new Map<string, { orgId: string; email: string; role: Role; status: string; expiresAt: number }>()
	...
	#standing(actor: string, orgId: string): TeamStanding {
		return standingOf(this.#role(actor, orgId), orgId !== CENTRAL && this.#role(actor, CENTRAL) !== null)
	}
	...
	async listOrgMembers(orgId: string) {
		return [...(this.members.get(orgId) ?? new Map()).entries()].flatMap(([userId, m]) => {
			...
			return user ? [{ userId, role: m.role, name: user.name, email: user.email, createdAt: m.createdAt }] : []
		})
	}
	async listUserOrgs(userId: string) {
		return [...this.members.entries()].filter(([, m]) => m.has(userId)).map(([organizationId]) => ({ organizationId }))
	}
	...
	async acceptOrgInvitation(userId: string, invitationId: string) {
		...
		if (invitation.status !== 'pending') return { ok: false as const, error: 'already-handled' as const }
		if (!user || user.email !== invitation.email) return { ok: false as const, error: 'wrong-email' as const }
		invitation.status = 'accepted'
		if (!this.#role(userId, invitation.orgId)) this.seat(invitation.orgId, user, invitation.role)
		return { ok: true as const, organizationId: invitation.orgId, role: this.#role(userId, invitation.orgId) ?? invitation.role }
	}
	...
	async manageTeamMember(actor: string, orgId: string, target: string, role: Role | null) {
		...
		const leads = [...(this.members.get(orgId)?.values() ?? [])].filter((m) => m.role === 'owner').length
		const verdict = mayChange({ standing: this.#standing(actor, orgId), self: actor === target, from, to: role, leads })
...
beforeAll(async () => {
	...
	await d
		...
		.values([doc(CORE_DOC, CENTRAL, 'core', 'cancer'), doc(PATHWAY_DOC, PATHWAY, 'pathway', 'breast cancer'), doc(OTHER_DOC, OTHER, 'pathway', 'lung cancer')])
})
...
describe('the roster', () => {
	it('lists the team leads first, with no address, for members and the central team', async () => {
		...
		expect(view.team).toEqual({ orgId: PATHWAY, name: 'Breast cancer', central: false, documentId: PATHWAY_DOC })
		expect(view.standing).toBe('member')
		expect(view.members.map((m) => m.name)).toEqual(['Lena Lead', 'Rae Reviewer', 'Dev Drafter', 'Vi Viewer'])
		...
		await expect(team.teamView(t, stranger.id, PATHWAY)).rejects.toThrow('You are not on this team.')
		await expect(team.teamView(t, elsewhere.id, PATHWAY)).rejects.toThrow('You are not on this team.')
	})
	...
	it('names the central team, which a steward does not manage by being one', async () => {
		...
		expect(view.team).toEqual({ orgId: CENTRAL, name: 'Cancer Australia', central: true, documentId: null })
		expect(view.standing).toBe('member')
	})
})
...
describe('who may change whom', () => {
	it('a reviewer changes drafters and viewers only', async () => {
		const t = teamEnv()
		expect(await team.changeMember(t, { actorId: reviewer.id, orgId: PATHWAY, userId: drafter.id, role: 'viewer' })).toEqual({
			name: 'Dev Drafter',
			role: 'viewer',
		})
		await expect(team.changeMember(t, { actorId: reviewer.id, orgId: PATHWAY, userId: drafter.id, role: 'admin' })).rejects.toThrow(
			'Reviewers can bring in and change drafters and viewers only.',
		)
		await expect(team.changeMember(t, { actorId: reviewer.id, orgId: PATHWAY, userId: lead.id, role: 'member' })).rejects.toThrow(
			'Reviewers can bring in and change drafters and viewers only.',
		)
	})

	it('a drafter changes no one', async () => {
		await expect(team.changeMember(teamEnv(), { actorId: drafter.id, orgId: PATHWAY, userId: viewer.id, role: 'member' })).rejects.toThrow(
	...
	it('the lead appoints another lead, then hands over and leaves; the last lead cannot', async () => {
		const t = teamEnv()
		await expect(team.changeMember(t, { actorId: lead.id, orgId: PATHWAY, userId: lead.id, role: null })).rejects.toThrow(
			'You are the only pathway lead. Appoint another pathway lead before you leave.',
		)
		await expect(team.changeMember(t, { actorId: lead.id, orgId: PATHWAY, userId: lead.id, role: 'admin' })).rejects.toThrow(
			'You are the only pathway lead. Appoint another pathway lead before you change your role.',
		)
		await team.changeMember(t, { actorId: lead.id, orgId: PATHWAY, userId: reviewer.id, role: 'owner' })
		expect(await team.changeMember(t, { actorId: lead.id, orgId: PATHWAY, userId: lead.id, role: null })).toEqual({ name: 'Lena Lead', role: null })
		await expect(team.changeMember(t, { actorId: steward.id, orgId: PATHWAY, userId: reviewer.id, role: null })).rejects.toThrow(
			'Rae Reviewer is the only pathway lead. Appoint another pathway lead first.',
		)
	})

	it('the central team manages any pathway team, appointing the lead included', async () => {
		const t = teamEnv()
		await team.changeMember(t, { actorId: steward.id, orgId: PATHWAY, userId: drafter.id, role: 'owner' })
		await team.changeMember(t, { actorId: steward.id, orgId: PATHWAY, userId: lead.id, role: 'viewer' })
		await team.changeMember(t, { actorId: steward.id, orgId: PATHWAY, userId: reviewer.id, role: null })
		expect((await team.teamView(t, drafter.id, PATHWAY)).members.map((m) => [m.name, m.role])).toEqual([
			['Dev Drafter', 'owner'],
			['Lena Lead', 'viewer'],
			['Vi Viewer', 'viewer'],
		])
	...
	it('anyone may leave', async () => {
		const t = teamEnv()
		await team.changeMember(t, { actorId: viewer.id, orgId: PATHWAY, userId: viewer.id, role: null })
		await team.changeMember(t, { actorId: reviewer.id, orgId: PATHWAY, userId: reviewer.id, role: null })
		expect((await team.teamView(t, lead.id, PATHWAY)).members.map((m) => m.name)).toEqual(['Lena Lead', 'Dev Drafter'])
	})
})
...
describe('after a change', () => {
	it('announces the row, or the removal, on the team topic', async () => {
		const t = teamEnv()
		await team.changeMember(t, { actorId: lead.id, orgId: PATHWAY, userId: drafter.id, role: 'admin' })
		await team.changeMember(t, { actorId: lead.id, orgId: PATHWAY, userId: viewer.id, role: null })
		expect(announced).toEqual([
			{ orgId: PATHWAY, change: { row: expect.objectContaining({ userId: drafter.id, role: 'admin', name: 'Dev Drafter' }) } },
	...
	it('re-tiers the person in the team’s documents from the access rule', async () => {
		const t = teamEnv()
		await team.changeMember(t, { actorId: lead.id, orgId: PATHWAY, userId: drafter.id, role: 'viewer' })
		expect(retiered).toEqual([{ room: `document:${PATHWAY_DOC}`, userId: drafter.id, tier: 'viewer' }])
		retiered = []
		...
		expect(retiered).toEqual([{ room: `document:${PATHWAY_DOC}`, userId: steward.id, tier: 'editor' }])
	})
	...
	it('a change to the central team re-tiers every document', async () => {
		...
		expect(rooms).toEqual(expect.arrayContaining([`document:${CORE_DOC}`, `document:${PATHWAY_DOC}`, `document:${OTHER_DOC}`]))
		// No role anywhere now: every room drops them to reading.
		expect(retiered.filter((r) => r.room === `document:${OTHER_DOC}`)).toEqual([{ room: `document:${OTHER_DOC}`, userId: steward.id, tier: 'viewer' }])
	})
})
...
describe('the ways in', () => {
	it('by email: an account is added and emailed', async () => {
		...
		const result = await team.addByEmail(teamEnv(), { actorId: lead.id, orgId: PATHWAY, email: elsewhere.email.toUpperCase(), role: 'member' })
		...
		expect(mail[0]?.text).toBe(`Lena Lead added you to Breast cancer as a drafter.\n\nhttp://test.local/d/${PATHWAY_DOC}`)
	})

	it('by email: an unknown address is invited, and accepts on the invitation page', async () => {
		const t = teamEnv()
		const address = `new-${run}@example.org`
		const result = await team.addByEmail(t, { actorId: reviewer.id, orgId: PATHWAY, email: address, role: 'viewer' })
		...
		expect(mail?.text).toContain('Rae Reviewer has invited you to work on Breast cancer as a viewer.')
		// Someone with another address cannot take it; the invited address can.
		await expect(team.acceptInvitation(t, { userId: stranger.id, invitationId: invitationId ?? '' })).rejects.toThrow('different email address')
		const invited = { id: `invited-${run}`, name: 'Ivy Invited', email: address }
		auth.users.set(invited.id, invited)
		expect(await team.acceptInvitation(t, { userId: invited.id, invitationId: invitationId ?? '' })).toEqual({
			documentId: PATHWAY_DOC,
			teamName: 'Breast cancer',
			role: 'viewer',
		})
		expect(announced.at(-1)).toEqual({ orgId: PATHWAY, change: { row: expect.objectContaining({ userId: invited.id, role: 'viewer' }) } })
		await expect(team.acceptInvitation(t, { userId: invited.id, invitationId: invitationId ?? '' })).rejects.toThrow('already been used')
	})

	it('by email: a lead cannot be invited, only appointed', async () => {
		await expect(team.addByEmail(teamEnv(), { actorId: lead.id, orgId: PATHWAY, email: `x-${run}@example.org`, role: 'owner' })).rejects.toThrow(
			'Someone new joins as a reviewer at most.',
		)
	})

	it('by member code: added and emailed; a wrong code is refused', async () => {
		const before = mailTo(stranger).length
		const result = await team.addByCode(teamEnv(), { actorId: reviewer.id, orgId: PATHWAY, code: `#${memberCodeOf(stranger.id).toUpperCase()}`, role: 'member' })
		expect(result).toEqual({ kind: 'added', name: 'Stan Stranger', role: 'member', emailed: true })
		expect(mailTo(stranger).slice(before).map((m) => m.subject)).toEqual(['You have been added to Breast cancer'])
		await expect(team.addByCode(teamEnv(), { actorId: reviewer.id, orgId: PATHWAY, code: 'zzzzzzzz', role: 'member' })).rejects.toThrow(
			'No one has that member code.',
		)
	})

	it('from the search: people on other OCP teams only, no address; added and emailed', async () => {
		const t = teamEnv()
		expect(await team.searchPeople(t, { actorId: lead.id, orgId: PATHWAY, query: 'eli' })).toEqual([{ id: elsewhere.id, name: 'Eli Elsewhere', image: null }])
		// By address too, never returned; not the stranger (on no team); not a member.
		expect(await team.searchPeople(t, { actorId: lead.id, orgId: PATHWAY, query: `elsewhere-${run}@` })).toHaveLength(1)
		...
		await expect(team.searchPeople(t, { actorId: drafter.id, orgId: PATHWAY, query: 'eli' })).rejects.toThrow('Only someone who can bring people in')
		const before = mailTo(elsewhere).length
		expect(await team.addPerson(t, { actorId: steward.id, orgId: PATHWAY, userId: elsewhere.id, role: 'admin' })).toEqual({
			kind: 'added',
			name: 'Eli Elsewhere',
			role: 'admin',
			emailed: true,
		...
		await expect(team.addPerson(t, { actorId: lead.id, orgId: PATHWAY, userId: stranger.id, role: 'member' })).rejects.toThrow(
			'That person is not on any pathway team yet.',
		)
	})

	it('someone already on the team stays as they are, and is not emailed', async () => {
		const before = mailTo(drafter).length
		expect(await team.addByEmail(teamEnv(), { actorId: lead.id, orgId: PATHWAY, email: drafter.email, role: 'viewer' })).toEqual({
			kind: 'already',
			name: 'Dev Drafter',
			role: 'member',
		})
	...
	it('a reviewer brings people in as drafters and viewers only', async () => {
		await expect(team.addByCode(teamEnv(), { actorId: reviewer.id, orgId: PATHWAY, code: memberCodeOf(stranger.id), role: 'admin' })).rejects.toThrow(
			'Reviewers can bring in and change drafters and viewers only.',
		)
	})
})
...
describe('invite links', () => {
	it('are made, listed, redeemed and revoked by those who may bring people in', async () => {
		const t = teamEnv()
		const link = await team.createInviteLink(t, { actorId: reviewer.id, orgId: PATHWAY, role: 'member' })
		expect(link.url).toBe(`http://test.local/join/${link.token}`)
		expect(link.expiresAt - link.createdAt).toBe(14 * 86_400_000)
		await expect(team.createInviteLink(t, { actorId: reviewer.id, orgId: PATHWAY, role: 'admin' })).rejects.toThrow('Reviewers can')
		await expect(team.createInviteLink(t, { actorId: drafter.id, orgId: PATHWAY, role: 'viewer' })).rejects.toThrow()
		expect((await team.listInviteLinks(t, { actorId: lead.id, orgId: PATHWAY })).map((l) => l.token)).toContain(link.token)
		await expect(team.listInviteLinks(t, { actorId: viewer.id, orgId: PATHWAY })).rejects.toThrow()

		expect(await team.inviteLinkInfo(t, { token: link.token })).toEqual({ valid: true, teamName: 'Breast cancer', role: 'member', central: false })
		...
		expect(announced.at(-1)).toEqual({ orgId: PATHWAY, change: { row: expect.objectContaining({ userId: stranger.id, role: 'member' }) } })
		// Redeemed again, or by a member: nothing changes.
		expect((await team.redeemInviteLink(t, { userId: lead.id, token: link.token })).already).toBe(true)
		...
		await expect(team.redeemInviteLink(t, { userId: newcomer.id, token: link.token })).rejects.toThrow('no longer valid')
		expect((await team.listInviteLinks(t, { actorId: lead.id, orgId: PATHWAY })).map((l) => l.token)).not.toContain(link.token)
	})

	it('expire after 14 days', async () => {
		const t = teamEnv()
		const made = Date.now() - 15 * 86_400_000
		const link = await team.createInviteLink(t, { actorId: lead.id, orgId: PATHWAY, role: 'viewer', now: made })
		expect(await team.inviteLinkInfo(t, { token: link.token })).toEqual({ valid: false })
		expect((await team.listInviteLinks(t, { actorId: lead.id, orgId: PATHWAY })).map((l) => l.token)).not.toContain(link.token)
		await expect(team.redeemInviteLink(t, { userId: newcomer.id, token: link.token })).rejects.toThrow('no longer valid')
		expect(await team.inviteLinkInfo(t, { token: link.token, now: made + 13 * 86_400_000 })).toMatchObject({ valid: true })
	})

	it('stop working when their maker loses the right to bring people in at their role', async () => {
		const t = teamEnv()
		const link = await team.createInviteLink(t, { actorId: reviewer.id, orgId: PATHWAY, role: 'member' })
		await team.changeMember(t, { actorId: lead.id, orgId: PATHWAY, userId: reviewer.id, role: 'viewer' })
		await expect(team.redeemInviteLink(t, { userId: newcomer.id, token: link.token })).rejects.toThrow('no longer works')
	})
	...
	it('only someone who may grant its role revokes a link', async () => {
		...
		await expect(team.revokeInviteLink(t, { actorId: reviewer.id, orgId: PATHWAY, token: link.token })).rejects.toThrow('Reviewers can')
...
describe('the central team’s first lead', () => {
	it('is the bootstrap’s caller, once', async () => {
		...
		expect((await team.teamView(t, steward.id, CENTRAL)).members).toEqual([expect.objectContaining({ userId: steward.id, role: 'owner' })])
