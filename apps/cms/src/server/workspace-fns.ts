...
async function requireMemberOf(userId: string, documentId: string) {
	...
	const document = (
		await d.select().from(schema.documents).where(eq(schema.documents.id, documentId)).limit(1)
	)[0]
	if (!document || !(await documentRoleOf(env.AUTH, d, userId, document.orgId)))
...
export const instructionsFor = createServerFn({ method: 'GET' })
	...
	.handler(
		async ({ data, context }): Promise<{ source: string; sections: InstructionsSection[] }> => {
							...
							await d
								...
								.where(
									and(
										...
										eq(schema.documents.templateId, document.templateId),
									),
								)
								.limit(1)
						)[0]
...
export const divergedFrom = createServerFn({ method: 'GET' })
	...
	.handler(
		async ({
			data,
			context,
		}): Promise<{ documentId: string; name: string; sectionId: string }[]> => {
			...
			const rows = await d
				...
				.where(
					and(
						eq(schema.sections.coreSectionId, data.sectionId),
						eq(schema.sections.ownership, 'owned'),
					),
				)
			return rows
				.map((r) => ({ documentId: r.documentId, sectionId: r.sectionId, name: documentName(r) }))
...
export const mentionable = createServerFn({ method: 'GET' })
	...
	.handler(
		async ({
			data,
			context,
		}): Promise<{ userId: string; name: string; team: 'document' | 'central' }[]> => {
			const document = await requireMemberOf(requireUser(context.userId), data.documentId)
			const { env, d } = await envOf()
			...
			const [own, centre] = await Promise.all([
				...
				central && central !== document.orgId
					? env.AUTH.listOrgMembers(central, OCP_NAMESPACE)
					: Promise.resolve([]),
			])
			...
			const add = (members: { userId: string; name: string }[], team: 'document' | 'central') => {
				for (const m of members)
					if (!out.some((o) => o.userId === m.userId))
						out.push({ userId: m.userId, name: m.name, team })
			}
			add(own, 'document')
			add(centre, 'central')
			return out.sort((a, b) =>
				a.team === b.team ? a.name.localeCompare(b.name) : a.team === 'document' ? -1 : 1,
			)
		},
	)
