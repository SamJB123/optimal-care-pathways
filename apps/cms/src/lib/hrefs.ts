/**
 * Where things are, as plain paths: the strings for what is not a route of the app's
 * router (files the worker serves, the API, anchors) and for code that runs with no
 * router at all — the seed scripts, the extractor, the worker. The typed router
 * destinations for in-app links are in links.ts, which builds on these; this module
 * must never import the router (the seed's import graph reaches it through the
 * template extractor).
 */
