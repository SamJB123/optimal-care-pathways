 ...
 *  leads no pathway. */
export const roleName = (role: Role, central: boolean): string =>
	central && role === 'owner' ? 'Lead' : ROLE_LABELS[role]

/** A role inside a sentence: "added as a drafter", "the only pathway lead". */
export const roleWord = (role: Role, central: boolean): string =>
	roleName(role, central).toLowerCase()
