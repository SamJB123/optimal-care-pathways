...
describe.skipIf(!existsSync(join(appDir, modelPath)))(
	'legacy import: breast cancer (second edition)',
	() => {
		const pathway = legacyBySlug('breast-cancer-2nd-edition')
		if (!pathway) throw new Error('catalogue entry missing')
		const core = mapTemplate({
			model: read('template/2026/extracted/cancer-template.model.json'),
			template: templateByKey('cancer-template'),
			orgId: 'central',
			id: deterministicId,
		})
		const result: LegacyImport = mapLegacy({
			model: read(modelPath),
			pathway,
			core,
			id: deterministicId,
			orgId: 'pending:breast-cancer',
			actorId: 'test',
		})
		...
		it('scaffolds the pathway from the template spine and keeps shared sections shared', () => {
			...
			for (const c of core.sections.filter((s) => !s.apparatus && !s.instructions))
				expect(addresses.has(c.address)).toBe(true)
			for (const c of core.sections.filter((s) => s.instructions))
				expect(addresses.has(c.address)).toBe(false)
			const shared = result.sections.filter((s) => s.ownership === 'shared')
			expect(shared.length).toBe(
				core.sections.filter(
					(s) => !s.apparatus && !s.instructions && s.pathwayOwnership === 'shared',
				).length,
			)
			...
			for (const s of result.sections)
				if (s.parentId) expect(result.sections.some((p) => p.id === s.parentId)).toBe(true)
		})

		it('places the legacy step sections the visual pass settled', () => {
			const to = (key: string) =>
				result.ledger.placements
					.filter((p) => p.legacyKey === key)
					.map((p) => `${p.how}:${p.destination}`)
			...
			for (const n of [2, 3, 4, 5, 6, 7])
				expect(to(`step-${n}/support-and-communication/supportive-care`)).toEqual([
					`title:${n}/supportive-care`,
				])
			...
			expect(
				result.sections.some(
					(s) => s.address === '1/introduction' || s.address === '1.1/prevention',
				),
			).toBe(false)
			...
			expect(
				to('step-3/support-and-communication/communication-with-patients-carers-and-families'),
			).toEqual(['rule:3/supportive-care'])
			expect(
		...
		it('turns timeframe subsections into timeframe boxes and checklists into point-of-care items', () => {
			...
			expect(
				result.ledger.timeframes.rows.every((r) => r.source === 'body' || r.destination !== null),
			).toBe(true)
			...
			expect(result.sections.find((s) => s.address === 'quick-reference-guide')?.pointOfCare).toBe(
				true,
			)
		})
		...
		it('carries the front matter as statements, nothing twice', () => {
			...
			expect(
				result.sections.find((s) => s.address === 'optimal-care-pathway-for-people-with')?.title,
			).toBe('Optimal care pathway for people with breast cancer')
			const edition = result.sections.find(
				(s) => s.address === 'optimal-care-pathway-for-people-with/x-edition',
			)
		...
		it('publishes the legacy edition as version 1 and opens version 2 as the draft', () => {
			...
			expect(JSON.stringify(v1Of('1')?.bodyJson)).toContain(
				'This step outlines recommendations for the prevention',
			)
