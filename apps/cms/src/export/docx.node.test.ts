...
const everything: JsonNode = {
	...
	content: [
		...
		p(
			...
			text(' [insert timeframe]', [
				{ type: 'placeholder', attrs: { label: '[insert timeframe]' } },
			]),
		...
		),
		{
			...
			content: [
				{ type: 'banner', attrs: { tone: 'band' }, content: [text('Box title')] },
				p(text('Inside the box.')),
			],
		},
		{
			type: 'box',
			attrs: { kind: 'callout', icon: '', family: 'info', variant: 'soft' },
			content: [p(text('A callout.'))],
		},
		...
		},
		{
			...
			content: [
				{
					type: 'tableRow',
					content: [cell('tableHeaderCell', 'Header A'), cell('tableHeaderCell', 'Header B')],
				},
				{
					type: 'tableRow',
					content: [
						cell('tableCell', 'Shaded', { background: 'warning' }),
		...
		item('bullet', [
			p(text('First bullet')),
			item('ordered', [p(text('Nested number'))], { order: 3 }),
		]),
		...
		{ type: 'image', attrs: { src: '/files/images/doc/drawing.svg', alt: 'A drawing' } },
		{
			...
			content: [
				{
					type: 'resource',
					attrs: { title: 'Cancer Council', url: 'https://www.cancer.org.au' },
					content: [p(text('Information and support.'))],
				},
...
async function build(sections: DocxSection[]) {
	...
	const file = await draftDocx({
		...
		references: [
			{
				number: 1,
				citation: 'First reference, viewed 1 May 2020, <www.example.org/one>.',
				url: null,
			},
...
describe('the draft as a Word file', () => {
	...
	it('writes every block of a core template, guidance included', async () => {
		const { files, read, document } = await build([
			{
				depth: 0,
				printedNumber: 'Step 1',
				title: 'Prevention',
				titleCitations: [],
				body: publishBody(everything, 'test cancer', 'template'),
			},
			{
				depth: 1,
				printedNumber: '1.1',
				title: 'Risk factors',
				titleCitations: ['r2'],
				body: { type: 'doc', content: [p(text('Short.'))] },
			},
		])
		...
		expect(document).toMatch(
			/<w:pStyle w:val="Heading2"\/>.*?<w:t[^>]*>1\.1 Risk factors<\/w:t>.*?<w:vertAlign w:val="superscript"\/><\/w:rPr><w:t[^>]*>2<\/w:t>/,
		)
		...
		expect(document).toMatch(
			/<w:highlight w:val="yellow"\/>.*?<w:t[^>]*> \[insert timeframe\]<\/w:t>/,
		)
		...
		expect(document).toMatch(
			/<w:tbl>(?:(?!<\/w:tbl>).)*<w:b\/>(?:(?!<\/w:tbl>).)*>Box title<\/w:t>(?:(?!<\/w:tbl>).)*>Inside the box\.<\/w:t>/,
		)
		// The template's guidance, shaded and headed.
		expect(document).toMatch(
			/<w:shd w:fill="E0F0E6"[^>]*\/>(?:(?!<\/w:tbl>).)*>Drafting guidance<\/w:t>(?:(?!<\/w:tbl>).)*>Write the subject here\.<\/w:t>/,
		)
		// The timeframe and its alternatives.
		expect(document).toContain('>Timeframe for referral</w:t>')
		expect(document).toMatch(
			/>Within two weeks\.<\/w:t>.*?>Or<\/w:t>.*?>Within four weeks\.<\/w:t>/,
		)
		...
		expect(document).toMatch(
			/<w:numPr><w:ilvl w:val="0"\/><w:numId w:val="\d+"\/><\/w:numPr>.*?>First bullet<\/w:t>/,
		)
		expect(document).toMatch(
			/<w:numPr><w:ilvl w:val="1"\/><w:numId w:val="\d+"\/><\/w:numPr>.*?>Nested number<\/w:t>/,
		)
		const numbering = read('word/numbering.xml')
		for (const glyph of ['✓', '✗', '☑'])
			expect(numbering).toContain(`<w:lvlText w:val="${glyph}"/>`)
		...
		expect(
			[...files.keys()].filter((name) => name.startsWith('word/media/')).length,
		).toBeGreaterThan(0)
		expect(document).toContain('descr="Figure A"')
		expect(document).toContain('>[A drawing]</w:t>')
		// The snapshot, off the timeframe boxes.
		expect(document).toMatch(
			/>Pathway step<\/w:t>.*?>Timeframe for referral<\/w:t>.*?>Within two weeks\.<\/w:t>.*?> or <\/w:t>.*?>Within four weeks\.<\/w:t>/,
		)
		...
		expect(document).toMatch(
			/>1\.<\/w:t><w:tab\/>.*?>First reference, viewed 1 May 2020, &lt;<\/w:t>.*?>www\.example\.org\/one<\/w:t>/,
		)
		...
		expect(document).toContain(
			'>Not for publication. Exported 24 September 2026 by Ada Drafter.</w:t>',
		)
		const headers = [...files.keys()]
			.filter((name) => /^word\/header\d+\.xml$/.test(name))
			.map(read)
			.join('')
		expect(headers).toContain('DRAFT — edition 3, not published · exported 24 September 2026')
		expect(read('docProps/core.xml')).toContain(
			'<dc:title>Optimal care pathway for people with test cancer</dc:title>',
		)
	})
		...
		const body: JsonNode = {
			...
			content: [
				p(
					...
					text(' should be seen.'),
					text(' <for this pathway only>', [{ type: 'instruction' }]),
				),
				{ type: 'guidance', attrs: { done: false }, content: [p(text('Write the subject here.'))] },
			],
		}
		const { document } = await build([
			{
				...
				title: 'Signs',
				titleCitations: [],
				body: publishBody(body, 'test cancer', 'pathway'),
			},
		])
